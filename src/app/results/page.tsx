...
export default async function ResultsPage({ searchParams }: PageProps) {
  ...
  const category = (sp.category ?? "").trim(); // ""ならすべて
  ...
  // ✅ まずは「カテゴリ文字列」をDBに保存している前提で絞り込み
  // まだDBに category が無い場合でもビルドは通るように (as any) でガード
  const where: any = {
  ...
  return (
    <main className="mx-auto w-full max-w-md px-4 py-8">
      <div className="rounded-2xl bg-pink-50 p-4 ring-1 ring-pink-100">
        ...
        <p className="mt-1 text-xs text-neutral-600">カテゴリで絞り込みできます</p>
      </div>
            ...
            <div className="flex items-center gap-2">
              <span
                className={[
                  ...
                  bloomBadgeClass(Number(s.bloom ?? 0)),
                ].join(" ")}
              >
                {bloomLabel(Number(s.bloom ?? 0))}
              </span>

              <span
                className={[
                  "rounded-full px-2.5 py-1 text-xs font-medium ring-1",
                  weatherBadgeClass((s.weather ?? null) as string | null),
                ].join(" ")}
              >
                {s.weather ?? "-"}
              </span>
